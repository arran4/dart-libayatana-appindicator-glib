...
void main() {
  test('updateMenuItems error handling', () async {
    ...
    expect(() => menu.updateMenuItems(0, position: 0, removeCount: -1), throwsRangeError);

    // Test 5: RemoveCount > length - position (RangeError)
    // Length 1. Position 0. Max removeCount is 1.
    expect(() => menu.updateMenuItems(0, position: 0, removeCount: 2), throwsRangeError);

    // Test 6: Happy Path - Update root items
    // Replace item at position 0.
    var newItem = DBusMenuItem(id: 2, properties: {'label': DBusString('Item 2')});
    ...
    var newSubItem = DBusMenuItem(id: 5, properties: {'label': DBusString('SubItem 2')});
    menu.updateMenuItems(3, position: 0, removeCount: 1, items: [newSubItem]);

    expect(parentItem.children.length, 1);
    expect((parentItem.children[0].properties['label'] as DBusString).value, 'SubItem 2');
    ...
    var deepItemReplacement = DBusMenuItem(id: 8, properties: {'label': DBusString('DeepItem 2')});

    // This should NOT throw ArgumentError if ID 6 is registered.
    menu.updateMenuItems(6, position: 0, removeCount: 1, items: [deepItemReplacement]);

    expect(deepChildren.length, 1);
    expect((deepChildren[0].properties['label'] as DBusString).value, 'DeepItem 2');
  });
}
